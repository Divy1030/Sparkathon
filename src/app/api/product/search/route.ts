...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${endpoints.product.search}?${queryString}`;
    ...
    return NextResponse.json({
      success: true,
      message: data.message || 'Products search completed successfully',
      data: data.data || data,
      pagination: data.pagination
    });
  } catch (error) {
    console.error('Search products error details:', error);
    return NextResponse.json(
      { 
        success: false, 
