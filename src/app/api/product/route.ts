...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = queryString ? `${endpoints.product.getAll}?${queryString}` : endpoints.product.getAll;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['name', 'sku', 'category', 'unitPrice'];
    ...
    // Call backend API
    const response = await fetch(endpoints.product.create, {
      method: 'POST',
      headers,
